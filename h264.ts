...
async function ls(folder: string, list: boolean) {
	...
	fs.readdirSync(folder).forEach(async (f) => {
		if (f.startsWith("$RECYCLE.BIN") ||
			f === 'System Volume Information') return;
...
async function compress(fPath: string, list: boolean) {
	...
	if (await deleteRename(fPath, nPath)) return;
...
async function tobin(path: string) {
	try {
...
async function deleteRename(fPath: string, nPath: string) {
	...
	if (fSize === 0) { await tobin(fPath); return false; }
	if (nSize === 0) { await tobin(nPath); return false; }
	...
	try { fLength = await getVideoDurationInSeconds(fPath); }
	catch (e) { console.log(`"${fPath} currupted. Error getting length."`.white.bgRed); await tobin(fPath); return false; }

	try { nLength = await getVideoDurationInSeconds(nPath); }
	catch (e) { console.log(`"${nPath} currupted. Error getting length."`.white.bgRed); await tobin(nPath); return false; }
	...
	if (isNaN(fLength) || isNaN(nLength)) {
		if (isNaN(fLength)) { console.log(`"${fPath} currupted. Error getting length."`.white.bgRed); await tobin(fPath); return false; }
		if (isNaN(nLength)) { console.log(`"${nPath} currupted. Error getting length."`.white.bgRed); await tobin(nPath); return false; }
	}
